...
ROWS = [
    ("host",),
    ("kv", "Focus", "Automated trading signals · tracking"),
    ("kv", "Cadence", "131 tickers · 3 scans daily · Mon–Fri"),
    ("gap",),
    ("sec", "Engine"),
    ("kv", "Signals", "Technical indicators + news sentiment"),
    ("kv", "Verdicts", "BUY / SELL / STAY, swing-to-position"),
    ("kv", "Paper", "Alpaca — automated since May 2026"),
    ("gap",),
    ("sec", "Surfaces"),
    ("bul", "Google Sheets · Slack · Gmail digests"),
    ("bul", "GitHub Pages live dashboard"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">hoophedge@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">hoophedge</tspan><tspan fill="{MUTED}">@</tspan>'
